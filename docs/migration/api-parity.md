...
| POST `/api/recipes/recommend` | AI recommendations, including prep context. | `POST /recipes/recommend` plus `POST /internal/recipes/recommend`. | required | user scoped AI request | migrated | Backend owns prompt, response schema validation, invented-ID filtering, and 30s user rate limit. MealPlanner keeps premium gate while billing/tier remains in web. |

### Meal Prep

| MealPlanner route | Behavior | Target backend route | Auth | Household/RLS | Status | Test coverage / next action |
|---|---|---|---|---|---|---|
| GET `/api/households/[id]/prep-batches` | List prep batches for a date window. | `GET /households/{householdId}/prep-batches?from=&to=`. | required | RLS via `caller_is_active_member()` | migrated | Implemented in `src/prep-batches.ts`; RLS-backed; tests in `test/prep-batches.test.ts`, including a two-member-same-household visibility case. |
| POST `/api/households/[id]/prep-batches` | Create prep batch with covered days/meals. | `POST /households/{householdId}/prep-batches`. | required | RLS via `caller_is_active_member()` | migrated | Validates recipeId XOR customRecipeId and assignment dates within 14 days of cookDate (matches MealPlanner's `ASSIGNMENT_WINDOW_DAYS`); inserts batch + assignments atomically via withRls. |
| DELETE `/api/households/[id]/prep-batches/[batchId]` | Delete prep batch. | `DELETE /households/{householdId}/prep-batches/{batchId}`. | required | RLS via `caller_is_active_member()` | migrated | Returns 404 for non-member or wrong-household batch; internal routes now call `assertMembership` same as public routes. |
...
- 2026-06-25: Prep-batch hardening from a prep-batch code review: (1) `0027_prep_batches_rls_caller_is_active_member.sql` rewrites all `household_prep_batches`/`household_prep_batch_assignments` RLS policies to use `caller_is_active_member()` (introduced in `0024` for `household_memberships` peer visibility) instead of the raw `EXISTS` subquery against `household_memberships` — consistency/defense-in-depth, not a confirmed live exploit (verified empirically against a local Postgres instance: the raw-EXISTS shape already correctly allowed same-household peer visibility because the EXISTS's own `household_id` filter, evaluated against the caller's own always-visible membership row, was sufficient). (2) `buildInternalPrepBatchesRoutes`'s DELETE batch and DELETE assignment handlers in `src/prep-batches.ts` now call `assertMembership` before touching the repository, matching every other route in the file. (3) `0028_prep_batches_meal_type_check.sql` adds a CHECK constraint restricting `household_prep_batch_assignments.meal_type` to `('lunch', 'dinner')`, matching the route-layer Zod enum; no FK added for `custom_recipe_id` because no `custom_recipes` table exists yet (forward-looking column for the not-yet-built feature in `custom-recipes-schema-and-api-spec-2026-04.md`). (4) `CreatePrepBatchSchema` in `src/prep-batches.ts` gained a `.refine()` rejecting assignment dates outside `[cookDate, cookDate + 14 days]`, matching MealPlanner's `ASSIGNMENT_WINDOW_DAYS` server-side check. Tests added in `test/prep-batches.test.ts` for all four.
