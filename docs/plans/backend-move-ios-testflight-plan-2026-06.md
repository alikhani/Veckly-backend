...
- 2026-08-02: Freemium Fas 2 released to production in shadow mode. Migrations `0035`–`0038` are applied and RLS-verified; gates remain disabled by default. Deployment `dpl_CVC6KKDTJE1VdEnWw88FW9BXmh5v` passed health, auth, and OpenAPI smoke tests. Backend builds now regenerate the committed OpenAPI artifact so Vercel cannot serve a stale static contract. StoreKit 2 is the next implementation slice.
