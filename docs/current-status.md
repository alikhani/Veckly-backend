...
### 2026-07-21 — Fix: household deletion was silently broken in production; RLS gap closed and shopping-scaling/test fixes deployed

`households` had RLS policies for select/insert/update but none for `DELETE` — `deleteHousehold` (shipped in commit `c796e9c`, Fas 6 of the iOS tightare-kärnupplevelse plan) has therefore never actually deleted a household for anyone; Postgres RLS defaults to deny with no matching policy, so every attempt silently affected 0 rows and the route correctly (if confusingly) returned `404 Household not found`. Found while fixing 3 pre-existing, unrelated backend test failures (stale `listRecipes`/`getRecipe`/`revokeInvite` call signatures) during iOS Fas 9 release verification. Fixed with `migrations/0032_households_delete_owner_only_rls.sql` (owner-only `DELETE`, mirrors the existing `households_update_owner_only` pattern) — applied directly to production via `mcp__supabase__apply_migration`, verified via `pg_policies` and `get_advisors` (no new security warnings).

This deploy also shipped a batch of 16 commits that had accumulated on `master` without being pushed/deployed: the Fas 2 shopping-list per-meal serving scaling (`scaledAmount = ingredientAmount × plannedMealServings ÷ recipeBaseServings`), a run of shopping-list dedup/caching/localization fixes, and an OpenAPI spec regeneration that picked up the `product-events` route (code had shipped earlier; the committed spec was stale). Deployed to production on 2026-07-21 as `dpl_9iNKE36SNrhpGw6tCezzDNMS5HUd` (`https://veckly-backend.vercel.app`) — verified serving the updated spec (`product-events` route present) immediately after.
